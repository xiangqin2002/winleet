				...
				if(!l.isEmpty()){
					for(Integer li : l){
						...
						if(!visited.contains(wordList.get(i))){
							q.add(new Pair(wordList.get(li), step + 1));
...
//			System.out.println(System.currentTimeMillis() - time1);
    ...
    private List<Integer> match(String content, int n, List<String> wordList){
    	List<Integer> ret = new ArrayList<>();
    	for(int i = 0; i < wordList.size(); i++){
//    		if(wordList.get(i).equals(content))
//    			continue;
    		...
    		for(int j = 0; j < length; j++){
    			if(j == n)
    				continue;
    			if(content.charAt(j) != wordList.get(i).charAt(j))
    				isMatch = false;
    		}
//    		String pattern = wordList.get(i).substring(0, n) + "." 
//    			+ wordList.get(i).substring(n + 1);
